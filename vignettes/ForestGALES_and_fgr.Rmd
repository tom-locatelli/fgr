...
```
A similar exercise with `fgr_constants` - firstly load the copy of the default values:
```
...
out_rou_mech <- as.data.frame(t(fg_rou_v(stand_id, date, species, mean_ht, mean_dbh, spacing, full_output, weib_a, weib_k, moe = moe, mor = mor)))
