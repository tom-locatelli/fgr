...
```
ex.stand_id <- as.character(df_rou$stand_id)
ex.date <- df_rou$date
ex.species <- as.character(df_rou$species)
ex.mean_ht <- df_rou$mean_ht
ex.mean_dbh <- df_rou$mean_dbh
ex.spacing <- df_rou$spacing
ex.full_output = 1
ex.weib_a <- df_rou$weib_a
ex.weib_k <- df_rou$weib_b
ex.moe <- df_rou$moe_by_age
ex.mor <- df_rou$mor_by_age

out_rou_mech <- as.data.frame(t(fg_rou_v(ex.stand_id, ex.date, ex.species, ex.mean_ht, ex.mean_dbh, ex.spacing, ex.full_output, ex.weib_a, ex.weib_k, moe = ex.moe, mor = ex.mor)))

library(tidyr)
out_rou_mech_df <- unnest(out_rou_mech)
```
Note that in the example above only the MOE (`ex.moe`) and MOR (`ex.mor`) are declared with their argument names (`moe` and `mor`), while all the other arguments are not. This is because all the other arguments are in the exact order and position as per the argument list in the `fg_rou` function call, while `moe` and `mor` are not.
...
Also note how all the variable names are stated out to make sure no unwanted misplacement of variables occurs.
```
...
For the purposes of this example, we are only concerned with the short output, so we can set `full_output` to 0 (i.e. "short" output). As for the previous *TMC* examples above, we want to treat `species` and `stand_id` as character type, not as factors. Note that information on individual and stand-level crown depth is available and therefore fed to the model. Tree positions are provided so they are included to allow the output to be applied eventually in a GIS project to map wind damge risk.
...
ex.full_output <- 0
ex.weib_a <- df_tmc_moses_gb$weib_a
ex.weib_k <- df_tmc_moses_gb$weib_b

out_tmc_moses_gb <- as.data.frame(t(fg_tmc_v(stand_id = ex.stand_id, tree_id = ex.tree_id, date = ex.date, species = ex.species, tree_ht = ex.tree_ht, dbh = ex.dbh, spacing_current = ex.spacing_current, predominant_species = ex.species, stand_mean_ht = ex.stand_mean_ht, stand_mean_dbh = ex.stand_mean_dbh, stand_top_ht = ex.stand_top_ht, full_output = ex.full_output, weib_a = ex.weib_a, weib_k = ex.weib_k, cr_depth = ex.cr_depth, stand_cr_depth = ex.stand_cr_depth, gap_size = ex.gap_size, x = ex.x, y = ex.y)))
...
As reported in the package's DESCRIPTION, we would like to draw attention to the licence applied to the *fgr* package, especially to its restriction to non-commercial use. The *fgr* package is released under Creative Commons' Attribution-NonCommercial-NoDerivatives 4.0 International (CC BY-NC-ND 4.0) (see https://creativecommons.org/licenses/by-nc-nd/4.0/ for further information). We encourage the use of the *fgr* package for academic and research work, and we welcome collaborative work that can make use of the package and the Tree Stability research team at Forest Research. Please feel free to contact any of the authors to discuss research and commercial collaboration/consultancy.
