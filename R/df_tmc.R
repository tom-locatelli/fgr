...
#'   \item{stand_mean_ht.}{ Mean tree height of the trees in the stand (m). Used to calculate the effect of edge and gap, and indirectly to elevate the critical wind speeds (CWS) to match anemometer data. Essential.}
...
#'   \item{weib_k.}{ Shape parameter (k) of the Weibull distribution of mean hourly wind speeds. This parameter (typically ranging between 1 and 3) is indicative of the variability of the mean wind speeds (small values indicating larger variability). Only used to calculate probabilities of damage, not the CWS. Defaults to 1.85.}
...
#'   \item{dist_edge.}{ Distance between an individual tree and the upwind stand edge (m). It defaults to 9 times \code{stand_mean_ht}. Desirable.}
#'   \item{gap_size.}{ Size of the upwind gap (m). Defaults to 0m (i.e. no gap), which effectively treats all trees as "middle of the stand" trees. Desirable.}
#'   \item{spacing_before.}{ Average spacing of trees before a thinning (m). It defaults to \code{spacing_current}. Desirable.}
...
#'   \item{moe.}{ Modulus of Elasticity (Young's modulus) of green wood (MPa). Defaults to species-specific values unless provided. Advanced.}
#'   \item{mor.}{ Modulus of Rupture of green wood (MPa). Defaults to species-specific values unless provided. Advanced.}
#'   \item{fknot.}{ Knot factor, scaled (0, 1] (dimensionless). It defaults to species-specific values unless provided. Advanced.}
#'   \item{stem_vol.}{ Volume of the stem (m3). Calculated with species-specific formulas and parameters unless provided. Advanced.}
#'   \item{crown_vol.}{ Volume of the crown (m3). Calculated with species-specific formulas and parameters unless provided. Advanced.}
#'   \item{stem_density.}{ Density of the green wood of the stem (kg m-3). It defaults to species-specific values unless provided. Advanced.}
#'   \item{crown_density.}{ Density of the crown (kg m-3). It defaults to species-specific values unless provided. Advanced.}
...
#'   \item{c_drag.}{ Value of the drag coefficient at rest (dimensionless). See the \code{Streamlining} documentation. It defaults to species-specific values unless provided. Advanced.}
#'   \item{n_drag.}{ Exponent of the power fit of drag to wind tunnel data. See the \code{Streamlining} documentation. It defaults to species-specific values unless provided. Advanced.}
#'   \item{drag_upper_limit.}{ Maximum wind speed used during the experiments from which \code{n_drag} and \code{c_drag} were derived (m*s-1). It defaults to species-specific values unless provided. Advanced.}
#'   \item{snow_depth.}{ Depth of snow layer on the canopy (m). It defaults to 0m (no snow). Advanced.}
#'   \item{snow_density.}{ Density of snow (kg m-3). It defaults to 150 kg m-3. Advanced.}
...
#'   \item{dams.}{ DAMS (Detailed Aspect Method of Scoring) value to describe the local wind climate (for UK conditions only). Only used to calculate probabilities of damage, not the CWS. Advanced.}
#' }
#'
#' @docType data
#' @keywords datasets
