...
#' @param tree_ht Individual tree height (m).
...
#' @param gap_size Length of the upwind gap (x).
...
#' @name TMC_Breakage_Critical_wind_speed_functions
NULL

#' @rdname TMC_Breakage_Critical_wind_speed_functions
uh_breakage_tmc <- function(tree_ht, dbh, cr_depth, cr_width, spacing_current, spacing_before, years_since_thin, dist_edge, gap_size,
                            moe, mor, fknot, stem_vol, stem_density, crown_density, snow_depth, snow_density, ci, ci_value,
                            n_drag, c_drag, drag_upper_limit, equivalent_mean_ht, stand_cr_depth, stand_cr_width, fgr_constants) {
  ...
  while (abs(uguess1 - uh_b) > fgr_constants$wind_precision) {
    ...
    tmr_full <- tm_ratio(spacing_before, spacing_current, years_since_thin, stand_cr_width, stand_cr_depth, uguess, n_drag, c_drag, drag_upper_limit, equivalent_mean_ht, fgr_constants)
    uh_b <- sqrt(breaking_moment / (tmc*dlf_used*tmr_full*edge_gap_factor))
    uguess <- uh_b
  }
  uh_b_results <- list(uh_b, tmc, tmr_full, dlf_calc, breaking_moment, edge_gap_factor, dlf_used)
  return(uh_b_results)
}

#Using tmr_simple####
#requires activating uh_breakage_tmc_tmr_simple function in fg_tmc.R script.
...
#' @rdname TMC_Breakage_Critical_wind_speed_functions
uh_breakage_tmc_tmr_simple <- function(tree_ht, dbh, cr_depth, cr_width, spacing_current, spacing_before, years_since_thin, dist_edge, gap_size,
                                       equivalent_mean_ht, moe, mor, fknot, stem_vol, stem_density, crown_density, snow_depth, snow_density, ci, ci_value, fgr_constants) {
  ...
  uh_b_results <- list(uh_b, tmc, tmr_simple, dlf_calc, breaking_moment, edge_gap_factor, dlf_used)
  return(uh_b_results)
}
