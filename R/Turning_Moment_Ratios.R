...
#' @rdname Turning_Moment_Ratios
tm_ratio <- function(spacing_before, spacing_current, years_since_thin, cr_width, cr_depth, uguess, n_drag, c_drag, drag_upper_limit, ht, fgr_constants) {#3
  if(years_since_thin > 5) {
    tmr_full <- 1
  } else {
    d_before <- zpd_fun(cr_width, cr_depth, spacing_before, uguess, n_drag, c_drag, drag_upper_limit, ht, fgr_constants)
    d_current <- zpd_fun(cr_width, cr_depth, spacing_current, uguess, n_drag, c_drag, drag_upper_limit, ht, fgr_constants)
    z0_before <- z0_fun(cr_width, cr_depth, spacing_before, uguess, n_drag, c_drag, drag_upper_limit, ht, fgr_constants)
    z0_current <- z0_fun(cr_width, cr_depth, spacing_current, uguess, n_drag, c_drag, drag_upper_limit, ht, fgr_constants)
...
#' @rdname Turning_Moment_Ratios
tm_ratio_ci <- function(spacing_before, spacing_current, years_since_thin, cr_width, cr_depth, uguess, n_drag, c_drag, drag_upper_limit, ht, ci, fgr_constants) { #4
  if(years_since_thin > 5) {
    tmrci_full <- 1
  } else if (ci == "none") {
    d_before <- zpd_fun(cr_width, cr_depth, spacing_before, uguess, n_drag, c_drag, drag_upper_limit, ht, fgr_constants)
